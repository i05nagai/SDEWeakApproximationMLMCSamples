/**
 * @file    european_option_blackscholes_mlmc_nv.c
 * @brief   level l estimation for Ninomiya-Victoir.

 *          level l estimation for Ninomiya-Victoir. 
...
/**
 * level l estimaton
 * level l esitmator using Ninoiya-Victoir
...
int mlmc_level_l_estimation_nv(SDE_MLMC *mlmc, MLMC_L *estimator, int l, int M, int N) {
	...
	if (l == 0) {
		...
		for (i=0; i < N; i++){
			...
			trans_rand_unif_to_normal(u_seq, rv_nv->rv_nv_n, n_dim);
